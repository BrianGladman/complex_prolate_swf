...
!  technical support from jeffrey boisvert. For more information see
...
!               of the order m and lnum values for the degree l = m, m+1,
!               ...,m+lnum-1 for a specified complex value of the size
!               parameter c.
...
!  assumed to be in the same directory as the executable file.
!  Cprofcn.dat contains the following lines of data.
!
!       line 1:
...
!                 : =1 if radial functions of the first kind
...
!          iopnorm: (integer)
!                 : =1 if angular functions of the first kind
!                      (and their first derivatives if iopang = 2)
...
!
!       line 3:
!          cc     : complex value of the size parameter c (= kd/2, where
!                   k = the complex  wavenumber and d = interfocal
!                   length) [complex(knd)] Note that complex numbers
!                   such as 10.0e0_knd+i1.0e0_knd are denoted by
...
!                   functions are to be computed. (real*16)
!
!          darg   : increment used to calculate additional desired
!                   arguments for angular functions. (real*16)
!
...
!
!  real(knd) scalars and complex(knd) scalar
!
...
!
        call main (mmin, minc, mnum, lnum, cc, ioprad, iopang, iopnorm, &
                   minacc, x1, ngau, ioparg, arg1, darg, narg, maxd, &
                   maxdr, maxint, maxj, maxlp, maxm, maxmp, maxn, maxp, &
                   maxpdr, maxq, maxt, neta, jnenmax, ndec, nex, kindd, kindq)
!
        end program
!
!
        subroutine main (mmin, minc, mnum, lnum, cc, ioprad, iopang, iopnorm, &
                         minacc, x1, ngau, ioparg, arg1, darg, narg, maxd, &
                         maxdr, maxint, maxj, maxlp, maxm, maxmp, maxn, maxp, &
                         maxpdr, maxq, maxt, neta, jnenmax, ndec, nex, kindd, &
                         kindq)
!
...
80          continue
            ...
            iopd = 0
            if(iopang == 2) iopd = 1
            call pleg(m, limps1, maxp, ndec, nex, limcsav, iopd, barg, narg, &
...
150           format(1x,'l = ',i6)
              ...
              limdneu = limd
              lplus = max(l, 1000)
              if(x1 >= 0.00065e0_knd) limdneu = 2 * ((lplus) * (-18.5q0- &
                                        20.0e0_knd * log10(x1)) &
                                        +5 * ndec + 4 * m + c + 01000)
              if(x1 > 0.08e0_knd) limdneu = 2 * ((lplus) * (0.5q0- &
                                        3.0e0_knd * log10(x1))+ &
                                        5 * ndec + 4 * m + c + 01000)
              if(x1 > 1.0e0_knd) limdneu = 2 * ((lplus) * 0.5q0 + 5 * ndec + 4 * m + c+ &
                                      00500)
              ...
              if(x1 > 0.08e0_knd) limdeta = 2 * ((lplus) * (0.5q0 - 3.0e0_knd* &
                                      log10(x1)) + 5 * ndec + 4 * m + c + 01000)
              if(x1 > 1.0e0_knd) limdeta = 2 * ((lplus) * 0.5q0 + 5 * ndec + 4 * m + c+ &
                                      00500)
...
!  calculation using integration technique
              ...
              if(igau == 0) call gauss(ngau, ndec, xr, wr)
              igau = 1
...
!  calculation using Legendre expansion and joining factor
              ...
              xin(1) = x
              limpleg = limdr + limdr
              call pleg(m, limpleg, maxp, ndec, nex, limcsav, iopd, xin, 1, maxt, &
...
370           if(ibflag2 == 1) go to 380
              ibflag2 = 1
              lp = max(lnum + m, 1000)
              limp = 2 * (lp * (-18.5q0 - 20.0e0_knd * log10(x1))+ &
                   5 * ndec + 4 * m + c + 01000)
              if(x1 > 0.08q0) limp = 2 * (lp * (0.5q0 - 3.0e0_knd * log10(x1))+ &
                                   5 * ndec + 4 * m + c + 01000)
              if(x1 > 1.0e0_knd) limp = 2 * (lp * 0.5q0 + 5 * ndec + 4 * m + c + 00500)
...
520             continue
              ...
              call pleg(m, limp, maxp, ndec, nex, limcsav, iopd, xlninp, &
              ...
              iopd = 2
              call pleg(m, limpd, maxp, ndec, nex, limcsav, iopd, etainp, &
                        netainp, maxt, prat, pdrat, pdnorma, ipdnorma, pnorma, &
                        ipnorma, alpha, beta, gamma, coefa, coefb, coefc, &
                        coefd, coefe)
              iopd = 3
...
610           continue
              nacccor = min(nacccor, nacceta - naccetao)
if (debug) then
                if(knd == kindd) then
...
715             format(1x,' est. acc. = ',i3,' digits for m = ',i5, ' l = ',i5,' x = ',f20.14,' c = ',f25.14, f25.14)
end if
                end if
              naccrp = naccr
              naccrplp = naccrpl
...
!  determine first kind prolate angular function
              ...
              call s1leg(l, m, cc, iopang, iopnorm, barg, narg, lims1, ndec, &
                ...
                do 810 jarg = 1, narg
if (debug) then                
                if(ioparg == 0 .and. iopang == 1) write(50, 730) &
                       arg(jarg), naccs(jarg)
                if(ioparg == 0 .and. iopang == 2) write(50, 735) &
                       arg(jarg), naccs(jarg), naccds(jarg)
730             format(1x,'theta = ',f19.14,'  accuracy = ',i3, &
                       ' digits.')
735             format(1x,'theta = ',f19.14,'  accuracies = ',i3, &
                       ' and',i3,' digits.')
                if(ioparg == 1 .and. iopang == 1) write(50, 740) &
                       barg(jarg), naccs(jarg)
                if(ioparg == 1 .and. iopang == 2) write(50, 745) &
                       barg(jarg), naccs(jarg), naccds(jarg)
...
if (output) then
                if(ioparg == 1 .and. iopang == 1) write(30, 750) &
                      barg(jarg), s1c(jarg), is1e(jarg), naccs(jarg)
                if(ioparg == 1 .and. iopang == 2) write(30, 760) &
                      barg(jarg), s1c(jarg), is1e(jarg), &
                      s1dc(jarg), is1de(jarg), naccs(jarg), naccds(jarg)
                if(ioparg == 0 .and. iopang == 1) write(30, 750) &
                      arg(jarg), s1c(jarg), is1e(jarg), naccs(jarg)
                if(ioparg == 0 .and. iopang == 2) write(30, 760) &
                      arg(jarg), s1c(jarg), is1e(jarg), &
                      s1dc(jarg), is1de(jarg), naccs(jarg), naccds(jarg)
...
!  backward summation of numerator series
        ...
        doldd = (1.0e0_knd, 0.0e0_knd)
          ...
          jj = j + j + ix
          rj = real(jj)
          dnew = dold * (jj - ix) / (sbesf(jj + m) * real((jj + mml)) * enr(j))
          dnewd = doldd * (jj - ix) / (sbesdf(jj + m) * real((jj + mml)) * enr(j))
          if(abs(dnew) > fterm) fterm = abs(dnew)
          if(abs(dnewd) > fdterm) fdterm = abs(dnewd)
          r1top = r1top + dnew
          r1dtop = r1dtop + dnewd
...
!  locate oblate-like eigenvalues
         ...
         limp = 0
           ...
           testpi = aimag(testp)
             do i = 1, imax
             if(abs((real(f(i)) - testpr) / real(f(i))) > 0.001q0) &
                go to 10
             if(abs((aimag(f(i)) - testpi) / aimag(f(i))) > 0.001q0) &
                go to 10
             limp = limp + 1
             fs(limp) = f(i)
             isav = i
...
20           il = max(isav - 1, 1)
             do i = il, isav
             if(abs((real(g(i)) - testpr) / real(g(i))) > 0.001q0) &
                 go to 30
             if(abs((aimag(g(i)) - testpi) / aimag(g(i))) > 0.001q0) &
                 go to 30
             gs(limp) = g(i)
               do kk = i, imax - limp
               g(kk) = g(kk + 1)
